...
export function MessagesPage() {
  ...
  async function handleSendText() {
    if (!selectedDeviceId) { toast.error('先选目标设备'); return }
    const t = text.trim()
    if (!t) { toast.error('消息不能为空'); return }
    setSubmitting(true)
    try {
      await sendText({ deviceId: selectedDeviceId, text: t })
      setText('')
    } catch (e) {
    ...
    <div className="grid h-full grid-cols-[240px_minmax(0,1fr)] gap-6 animate-fade-in overflow-hidden">
      ...
      <div className="h-full overflow-y-auto py-6 pr-8 pl-1 space-y-5 scrollbar-thin">
        ...
        <div className="rounded-xl border bg-[hsl(var(--panel))]">
          <div className="px-5 pt-4 pb-3 text-[11px] font-medium uppercase tracking-widest text-[hsl(var(--muted))]">消息记录</div>
